...
# 初始化日志文件
logger 'init'
logger 'file' "已初始化 日志函数"

# 加载一些小东西
# 输出数组内容
source $ShellFilePath/lib/println_array_items.sh && logger file "已加载 读出数组函数"
# 打印分割线
source $ShellFilePath/lib/separator.sh && logger file "已加载 分割线函数"
# 读取初始化函数
source $ShellFilePath/init.sh && logger file "已加载 初始化函数"
# 读取备份逻辑函数
source $ShellFilePath/backup.sh && logger file "已加载 备份逻辑函数"
# 读取恢复函数
source $ShellFilePath/restore.sh && logger file "已加载 恢复逻辑函数"
...
# 记录处理后用户输入的参数
logger 'file' "整理输入参数成功，整理后用户输入的参数列表为 [$@]"
# 格式化getopt的输出
...
do
    case $1 in
        ...
        ;;
        --)
            logger 'file' "无参数输入，打印帮助信息"
            # 打印一个提示信息
            cat $ShellFilePath/help_info
            exit
        ;;
        *)
            logger 'file' "参数不是小功能，准备进入第二个环境的循环"
            break
        ;;
    esac
done
