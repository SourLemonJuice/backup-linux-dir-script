...
do
    case $1 in
        ...
        -B)
            ...
            {
            cd $RootPath
            tar -g $BackupFolder/snapshot -zcvf $BackupFolder/$(date +%Y-%m-%d_%H-%M-%S)_backup.tar.gz --exclude=lost+found --exclude=/sys --exclude=/proc --exclude=/boot --exclude=/dev --exclude=/mnt .
            }
        ;;
        -r)
            # 列出所有可用的备份
